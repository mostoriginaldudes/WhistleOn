...
public class JwtInterceptor implements HandlerInterceptor {
  ...
  /**
   * JwtInterceptor is a class to handle login as Interceptor using JWT token. We use preHandle
   * method to validate the token before accessing the controller. If token is not null and usable,
   * method returns true. Otherwise, send redirect to login page.
   *
   * @param request
   * @param response
   * @param handler
