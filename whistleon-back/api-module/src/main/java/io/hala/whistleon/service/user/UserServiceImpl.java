...
import io.hala.whistleon.domain.user.UserInfo;
...
public class UserServiceImpl implements UserService {
  ...
  public void updateUserInfo(String email, UpdateUserInfoRequestDto updateUserInfoRequestDto) {
    ...
    if (matchesEmail(userEmail, email)) {
      UserInfo userInfo = userInfoFromReq(updateUserInfoRequestDto);
      checkExistNickname(userInfo.getNickname());

      User user = userRepository.findByEmail(email)
          .orElseThrow(() -> new CustomException(ExceptionCode.RESOURCES_NOT_EXIST));

      user.updateUserUsingUpdateInfo(userInfo);
    }
  }
  ...
  private UserInfo userInfoFromReq(UpdateUserInfoRequestDto requestDto) {
    return UserInfo.builder()
        ...
        .position1(requestDto.getPosition1())
        .position2(requestDto.getPosition2())
